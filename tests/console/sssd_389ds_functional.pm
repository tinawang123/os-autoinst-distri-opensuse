...
sub setup_389ds_container ($container_engine) {
    ...
    script_run("mkdir /tmp/sssd && cd /tmp/sssd");
...
sub configure_sssd_client ($container_engine, $run_as_user = 'root') {
	...
	assert_script_run("chmod 0600 /etc/sssd/sssd.conf");
        ...
        my $override = "[Service]\nUser=sssd\nGroup=sssd\nSupplementaryGroups=";
        assert_script_run("echo -e \"$override\" > /etc/systemd/system/sssd.service.d/override.conf", timeout => 500);
    } else {
...
sub run ($self) {
	...
	script_run("$container_engine inspect -f \'{{.State.Running}}\' ds389_container");
