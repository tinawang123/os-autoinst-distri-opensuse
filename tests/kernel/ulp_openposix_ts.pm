...
sub setup_ulp {
    ...
    install_klp_product;
    install_package('libpulp0 libpulp-tools libpulp-load-default', trup_reboot => 1);

    # Find glibc versions targeted by livepatch package
    my $provides = script_output("zypper -n info --provides $repo_args $packname");
    #my @versions = $provides =~ m/^\s*libc_([^_()]+_[^_()]+)_livepatch\d+\.so\(\)\([^)]+\)\s*$/gm;
